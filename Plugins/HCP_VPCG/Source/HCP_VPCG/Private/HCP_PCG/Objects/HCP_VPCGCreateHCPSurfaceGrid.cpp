...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HCP_VPCGCreateHCPSurfaceGrid)

#define LOCTEXT_NAMESPACE "HCP_VPCG_Surface"
#define PCGEX_NAMESPACE CreateBuilder_hcp_vpcg_surface
...
void UHCP_VPCGCreateHCPSurfaceGridSettings::PostLoad()
{
	Super::PostLoad();

#if WITH_EDITOR
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
		if (GridPivot_DEPRECATED != EHCP_VPCGSurfaceGridPivot::Global)
		{
			CoordinateSpace = static_cast<EPCGCoordinateSpace>(static_cast<int8>(GridPivot_DEPRECATED));
			GridPivot_DEPRECATED = EHCP_VPCGSurfaceGridPivot::Global;
		}
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
#endif
}

TArray<FPCGPinProperties> UHCP_VPCGCreateHCPSurfaceGridSettings::InputPinProperties() const
{
	return TArray<FPCGPinProperties>();
}

FPCGElementPtr UHCP_VPCGCreateHCPSurfaceGridSettings::CreateElement() const
{
	return MakeShared<FHCP_VPCGCreateSurfaceGridElement>();
}

bool FHCP_VPCGCreateSurfaceGridElement::ExecuteInternal(FPCGContext* Context) const
{
	...
	const UHCP_VPCGCreateHCPSurfaceGridSettings* Settings = Context->GetInputSettings<UHCP_VPCGCreateHCPSurfaceGridSettings>();
	check(Settings);
	...
	if (Settings->GridExtents.X < 0.0 || Settings->GridExtents.Y < 0.0)
	{
	...
	FVector GridExtents = FVector(Settings->GridExtents.X, Settings->GridExtents.Y,1);
...
UPCGData* GetDependenciesDataHCPSurface(IPCGGraphExecutionSource* InExecutionSource, EPCGCoordinateSpace InCoordinateSpace, bool bInCullPointsOutsideVolume)
{
...
void FHCP_VPCGCreateSurfaceGridElement::GetDependenciesCrc(const FPCGGetDependenciesCrcParams& InParams, FPCGCrc& OutCrc) const
{
	FPCGCrc Crc;
	IPCGElement::GetDependenciesCrc(InParams, Crc);

	if (const UHCP_VPCGCreateHCPSurfaceGridSettings* Settings = Cast<UHCP_VPCGCreateHCPSurfaceGridSettings>(InParams.Settings))
	{
		int CoordinateSpace = static_cast<int>(EPCGCoordinateSpace::World);
		bool bCullPointsOutsideVolume = false;
		PCGSettingsHelpers::GetOverrideValue(*InParams.InputData, Settings, GET_MEMBER_NAME_CHECKED(UHCP_VPCGCreateHCPSurfaceGridSettings, CoordinateSpace), static_cast<int>(Settings->CoordinateSpace), CoordinateSpace);
		...
		if (UPCGData* Data = GetDependenciesDataHCPSurface(InParams.ExecutionSource, EnumCoordinateSpace, bCullPointsOutsideVolume))
		{
